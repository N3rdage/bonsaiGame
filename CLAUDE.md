...
- **Branch shape lives in `branch_frame_at(_tree, _branch, _t)`.** Branches carry a single `branch.bend` scalar interpreted as the total angular sweep over the branch's length, with bend axis = world +z. That uniform-curvature constraint admits a closed-form circular-arc integral for position, and direct rotation formulas for the frame at any `_t` — no walk, no array, no Rodrigues iteration. `branch_point` is the position-only hot path (called per-frame for hotspots); `branch_frame_at` adds tangent/normal/binormal for callers that need the full basis (currently `add_wire_coil`, so the helix follows the curve). Same "what you see is what you score" principle as trunks, though no scoring criterion currently reads `branch.bend`.
