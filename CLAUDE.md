...
- **Mesh rebuilds are triggered by `mesh_dirty`.** Any operation that mutates tree morphology (growth, wire, clip, prune, repot) must set `tree.mesh_dirty = true` (or call `mark_dirty()` / use `add_branch` which does it). The viewer rebuilds lazily via `get_mesh()`, which returns a struct `{ bark, foliage }` of two frozen vertex buffers. Each draw site does two `vertex_submit` calls — bark untextured, foliage textured + alpha-cutoff (PR2). Cache invalidation must free both.
