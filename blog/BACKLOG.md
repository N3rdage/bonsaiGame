...
1. **[Math] Why the wired branch still looks straight** — teased at the end of the trunk-bending post. `branch.bend` rotates the direction vector but the branch geometry stays a straight line. Same maths family as the (shipped) trunk parallel-transport, but a branch carries a single bend scalar rather than a list of events — the difference is more interesting than it sounds. Sets up TODO #9 ("proper branch curvature").
...
3. **[Math] Where does branch 3 actually start?** — the trunk-surface offset, the `branch_point(tree, branch, t)` helper. The trunk-attachment maths skipped over by the trunk-bending post: a branch's attachment angle expressed in the trunk's local basis (now that the trunk has one), plus the trunk's radius at the branch's height. Bridges from "circle in the abstract" to "circle as the cross-section of a real game object whose orientation can change."
4. **[Math] Wrapping wire round a branch** — local frames perpendicular to a branch direction; helix parametrisation; oriented rings via a cross-product basis. The maths from the wire-coil PR (#19). Linear algebra starts to earn its keep.
5. **[Math] Z-up in a y-down world (deeper cut)** — projection matrix sign flips, lookat conventions, why GameMaker's 3D tutorials get this subtly wrong for non-toy scenes. Partial overlap with the upside-down-tree post; this version goes further into the linear algebra. Topical hook: the trunk-bending UX shipped with screen-relative direction buttons that turned out to be Left/Right-swapped because of exactly this — a worked example of the handedness flip biting in practice.
6. **[Math] What scoring 100 actually means** — the scoring function as a maths object. Weighted average of bounded criteria, vitality multiplier, style conformance as a plug-in. Less geometry, more "where does the number on the inspector come from." Lands well after the geometric posts have built up reader confidence.
